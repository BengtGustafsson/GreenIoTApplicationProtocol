...
struct InfoReply {
	...
	double mMaxResolution;			///< Suggested tightest grid spacing for RectRegion requests, in meters. Typically the server will not deliver tighter grids even if asked.
	double mNoDataValue;			///< The no-data value indicator used by the server (can only be sent instead of double data).
...
/// Abstract base class of all types of geographical regions.
...
/// and moving sensors in the area within the requested TimeSpecification (mounted on for instance vehicles). For moving sensors the reply will contain the individual longlat
...
/// coordinate system and then subsample this grid using the mResolution hint. If the server uses a precalculated grid of points it
...
/// If the operation is not cNone a set of data bins is created depending on the selected Interval period. The number of data
/// bins is selected automatically depending on the Interval. Each bin contains the result of subjecting the samples of each
/// subinterval to the operation. For instance if cHour interval is selected with cMax Operation the result in each of 24 bins is
/// the highest value in that hour for any of the days included in the overall time interval selected by the TimeSpecification. Note that even if
/// the TimeSpecification is shorter than the interval period the full number of bins will be returned, while some of the data
/// values are set to InfoReply::mNoDataValue.
...
/// All requests are represented by a DataRequest object. To do a request create a JSON encoded representation of one of these and
/// Post to the URL returned in InfoReply::mDataURL.
struct DataRequest {
...
struct DataReply {
	...
	std::vector<double> mData;		///< The vector contains the values at each bin/time. -1 
};
...
/// MovingSensorReply is returned for moving sensors when cRawValues Interval and a SensorRegion or SensorIdRegion for
...
struct DataReplyContainer {
	...
	DateTime mFirstSampleTime;					///< Time for first sample in each vector in the DataReplies.
	double mSampleInterval;						///< Interval between samples in seconds.
	std::vector<DataReply*> mReplies;			///< vector of replies in row major order (x coordinate varies fastest).
};


// BeamEd3D can export a longlat grid of any resolution for an Encitra project. It defines the longlat coordinates of each point in
// a grid of points. The resolution of the grid must be an even fraction of the 768 m square size of the Encitra regions. The
// LongLatGrid is used in Unity to convert with decent precision longlat coordinates to local coordinates. 0,0 of the grid always
// contains the longlat of the 0,0 coordinate of the Encitra model or unity export.
struct LongLatGrid {
	double mGridSpacing;
	std::vector<std::vector<LongLat>> mCoordinates;
};
